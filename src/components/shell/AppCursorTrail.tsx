...
/* Ambient turquoise fluid cursor trail for the app shell — adapted from the
 * landing CursorTrail (Three.js ping-pong feedback buffer with curl noise).
 * Product register: quieter opacity, auto-mutes while the pointer is down so
 * it never fights chart brushing / drag interactions. Desktop fine pointers
 * only; never rendered under prefers-reduced-motion.
 */
...
export default function AppCursorTrail() {
  ...
  useEffect(() => {
    ...
    /* Mute while dragging / brushing charts — motion must never fight input. */
    const handlePointerDown = () => setMuted(true)
    const handlePointerUp = () => setMuted(false)
