...
export function mergePlayersByName(players: Player[], name: string): Player | null {
  ...
  return {
    ...
    gd15: merged.gd15.length ? round(avgWeighted(merged.gd15), 1) : undefined,
    csd15: merged.csd15.length ? round(avgWeighted(merged.csd15), 1) : undefined,
    xpd15: merged.xpd15.length ? round(avgWeighted(merged.xpd15), 1) : undefined,
