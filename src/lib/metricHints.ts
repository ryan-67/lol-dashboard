export const PERFORMANCE_SCORE_HINT =
  'Per-game composite score: role-specific stats are min–max normalized against the same-role split cohort and weighted by role. Stats missing for a league or player (e.g. LPL @15 gaps) are omitted from the score rather than treated as zero; partial stats count only when present on at least half of games. Player of the Week is the highest weekly average. Scale: 0–100.'
