 ...
 * noise advection, recoloured to the nucky turquoise signal palette.
 *
 * Shared by the landing hero and the product shell so both surfaces move with
 * exactly the same hand. The canvas is screen-blended over the page, so pure
 * black in the buffer stays invisible and only the signal reads.
 */
...
void main() {
  ...
  t2 = pow(t2, 10.);
  t3 = pow(t3, 4.);
  float scale = speed * 5. * uGain;
  t *= scale;
  t2 *= scale;
  t3 *= scale;

  /* nucky signal palette: deep teal wash -> turquoise core -> pale hot centre */
...
export interface CursorTrailOptions {
  /** Signal strength of the injected head. Landing 1.0, product ~0.85. */
  gain?: number
  /** Buffer resolution scale. Lower = softer + cheaper. */
  resolutionScale?: number
}

/**
 * Mount the trail on a canvas. Returns a disposer, or null when WebGL is
 * unavailable. Callers own the "should this run at all" decision.
 */
export function createCursorTrail(
  canvas: HTMLCanvasElement,
  options: CursorTrailOptions = {},
): (() => void) | null {
  const { gain = 1, resolutionScale = 0.25 } = options
  ...
  const outputMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: { uTrailMap: new THREE.Uniform<THREE.Texture | null>(null) },
    depthWrite: false,
  })
  ...
  const bufferSize = () =>
    [
      Math.max(2, Math.floor(sizes.width * resolutionScale)),
      Math.max(2, Math.floor(sizes.height * resolutionScale)),
    ] as const
  ...
  const [bw, bh] = bufferSize()
  const trailMaterial = new THREE.ShaderMaterial({
    vertexShader: TRAIL_VERTEX,
    fragmentShader: TRAIL_FRAGMENT,
    uniforms: {
      uResolution: new THREE.Uniform(new THREE.Vector2(bw, bh)),
